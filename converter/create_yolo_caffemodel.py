...
def main(argv):
    ...
    try:
        ...
        print(
            'create_yolo_caffemodel.py -m <model_file> -w <yoloweight_filename> -o <caffemodel_output>'
        )
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
    ...
    netWeights = netWeightsFloat[
        start:]  # start from the 5th entry, the first 4 entries are major, minor, revision and net.seen
    print(netWeights.shape)
    count = 0

    print("#Total Net Layer %d" % len(net.layers))
    ...
    for pr in params:
        ...
        if count == netWeights.shape[0] and (layer.type != 'BatchNorm'
        ...
        if layer.type == 'Convolution':
            ...
            conv_bias = np.reshape(netWeights[count:count + biasSize],
                                   bias_dim)
            if len(net.params[pr]) > 1:
            ...
            if lidx + 1 < len(
                ...
                batch_norm = np.reshape(netWeights[count:count + bnSize],
                                        bn_dims)
                count = count + bnSize
            # weights
            ...
            net.params[pr][0].data[...] = np.reshape(
                netWeights[count:count + weightSize], dims)
            ...
            net.params[pr][1].data[...] = np.reshape(
                netWeights[count:count + biasSize],
            ...
            if transFlag:
                net.params[pr][0].data[...] = np.reshape(
                    netWeights[count:count + weightSize],
