...
#     raise Exception("This script is only compatible with Python 2.")
import os
import argparse
import logging
...
class CaffeConvolutionLayer(CaffeLayerGenerator):
    def __init__(self,
        ...
        self.stride = stride
        self.pad = pad
        # Fix for point-wise convolution layers with pad == '1'
        if ksize == '1' and pad == '1':
            self.pad = '0'
        self.bias = bias
    ...
    def write(self, f):
        ...
        if self.ksize is not None:
            opts.append('kernel_size: {}'.format(self.ksize))
        if self.stride is not None:
            opts.append('stride: {}'.format(self.stride))
...
class CaffeDeconvolutionLayer(CaffeLayerGenerator):
    def __init__(self,
                 name,
                 filters,
                 groups=None,
                 ...
                 pad=None,
                 bias=True):
        super(CaffeDeconvolutionLayer, self).__init__(name, 'Deconvolution')
        self.filters = filters
        self.groups = groups
        self.ksize = ksize
        self.stride = stride
    ...
    def write(self, f):
        ...
        if self.groups is not None:
            opts.append('group: {}'.format(self.groups))
        if self.pad is not None: opts.append('pad: {}'.format(self.pad))
        if not self.bias: opts.append('bias_term: false')
        param_str = """
  convolution_param {{
...
class CaffePoolingLayer(CaffeLayerGenerator):
    ...
    def write(self, f):
        ...
        if self.global_pooling is not None:
            opts.append('global_pooling: {}'.format(
                'True' if self.global_pooling else 'False'))
        param_str = """
  pooling_param {{
    pool: {}{}
...
class CaffeProtoGenerator:
    def __init__(self, name):
        self.name = name
        self.sections = []
        self.main_section_mask = []
        self.lnum = 0
        self.layer = None

    def add_layer(self, l):
        is_main_section = False
        if type(l) in [CaffePoolingLayer, CaffeConvolutionLayer]:
            is_main_section = True
        self.sections.append(l)
        self.main_section_mask.append(is_main_section)

    def add_input_layer(self, items):
        self.lnum = 0
        lname = "data"
        self.layer = CaffeInputLayer(lname, items['channels'], items['width'],
                                     items['height'])
    ...
    def add_convolution_layer(self, items):
        ...
        bias = not bool(
            items['batch_normalize']) if 'batch_normalize' in items else True
        self.layer = CaffeConvolutionLayer(
            lname, filters, ksize=ksize, stride=stride, pad=pad, bias=bias)
    ...
    def add_power_layer(self, items, scale=0.08):
        prev_blob = self.layer.bottom[
            0]  # not the top since it use same previous blob
        prev_relu = self.layer.top[0]  # save the same level relu
        lname = "power" + str(self.lnum)
        self.layer = CaffePowerLayer(lname, scale)
        self.layer.bottom.append(prev_blob)
        self.layer.bottom.append(
            prev_relu
        )  # save the same level relu, not gonna created since create_layer only use index 0
        self.layer.top.append(lname)
        self.add_layer(self.layer)
...
def convert(cfgfile, ptxtfile, no_leaky):
        ...
        if _section == 'net':
            ...
            if relu_followed:
                if no_leaky:
                    ...
                    gen.add_leaky_relu_layer(
                        items)  #assume all pre trained yolo model use leaky
            ...
            logging.warning(
                "{} layer is not supported, will not add it to prototxt".
                format(_section))
...
def main(args):
    ...
    convert(args.cfg, args.prototxt, args.noleaky)


if __name__ == "__main__":
    argparser = argparse.ArgumentParser(
        description='Convert YOLO cfg to Caffe prototxt')
    argparser.add_argument(
        '-c', '--cfg', type=str, required=True, help='YOLO cfg')
    argparser.add_argument(
        '-m', '--prototxt', type=str, required=True, help='Caffe prototxt')
    argparser.add_argument(
        '--noleaky',
